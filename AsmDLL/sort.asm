...
$LN7:
        ...
        mov     DWORD PTR j$1[rsp], eax             ; assigning this value to j variable which will be used in loop
        jmp     SHORT $LN4@partition
$LN2@partition:                                     ; incrementing loop counter (j)
        mov     eax, DWORD PTR j$1[rsp]             ; load j to registry
        inc     eax                                 ; increment
        mov     DWORD PTR j$1[rsp], eax             ; save to variable
$LN4@partition:                                     ; checking the loop condition - j =< high - 1
        mov     eax, DWORD PTR high1$[rsp]          ; load high value to registy
        dec     eax                                 ; decrement its value
        cmp     DWORD PTR j$1[rsp], eax             ; compare j and high - 1
        jg      SHORT $LN3@partition                ; if j > high - 1 end loop
        movsxd  rax, DWORD PTR j$1[rsp]             ; loading j value to registry
        ...
        jg      SHORT $LN5@partition                ; jump to end of the loop iteration if arr[j] > pivot
        ...
        mov     DWORD PTR t$2[rsp], eax             ; saving value of arr[i] in temp variable used for swap
        movsxd  rax, DWORD PTR j$1[rsp]             ; read j value
        ...
        movsxd  rax, DWORD PTR j$1[rsp]             ; rad j value
        mov     rcx, QWORD PTR arr$[rsp]            ; read arr 
        mov     edx, DWORD PTR t$2[rsp]             ; read t value
