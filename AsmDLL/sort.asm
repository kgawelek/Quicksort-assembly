...
Sort proc
start:
        mov     DWORD PTR [rsp+24], r8d             ; load argument - high_index
        mov     DWORD PTR [rsp+16], edx             ; load argument - low_index
        mov     QWORD PTR [rsp+8], rcx              ; load argument - arr
        sub     rsp, 20                             ; allocates part of stack for variables
        movsxd  rax, DWORD PTR high_index[rsp]      ; reading high value to use it for pivot
        mov     rcx, QWORD PTR arr[rsp]             ; reading arr address
        mov     eax, DWORD PTR [rcx+rax*4]          ; eax=arr[high] which will be a pivot element
        mov     DWORD PTR pivot[rsp], eax           ; loading this value to pivot variable
        mov     eax, DWORD PTR low_index[rsp]       ; reading low value
        dec     eax                                 ; decrementing low value
        mov     DWORD PTR i[rsp], eax               ; loading (low - 1) value to i variable
        mov     eax, DWORD PTR low_index[rsp]       ; reading low value which will be used for loop
        mov     DWORD PTR j[rsp], eax               ; assigning this value to j variable which will be used in loop
        jmp     SHORT inside_loop                   ; go to loop
j_increment:                                        ; incrementing loop counter (j)
...
inside_loop:                                        ; checking the loop condition - j =< high - 1
        mov     eax, DWORD PTR high_index[rsp]      ; load high value to registy
...
after_loop:
        ...
        movsxd  rax, DWORD PTR high_index[rsp]      ; read high value
        ...
        add     rsp, 20                             ; return top of the stack to the point from beginning
        ret     
Sort endp
end